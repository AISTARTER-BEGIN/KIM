 ...
 * Step 2: 선택 옵션 기반 가중치 연산 및 결과 렌더링
 *
 * 구조:
...
(function () {
    ...
    const PRICING_CONFIG = {
        ...
        techs: {
            ...
            data_pipeline:      { label: '데이터 파이프라인',     duration: 2, cost: 1_500_000 },
            dashboard:          { label: '모니터링 대시보드',     duration: 2, cost: 1_000_000 },
        },

        /** 플랫폼 가중치 — 단일 선택, 필수 */
        platforms: {
            rpi:       { label: 'Raspberry Pi',       duration: 1, cost:   500_000 },
            jetson:    { label: 'NVIDIA Jetson',       duration: 2, cost: 1_500_000 },
            x86_linux: { label: 'x86 Linux 서버',     duration: 0, cost:         0 },
            cloud:     { label: '클라우드 (AWS/GCP)', duration: 1, cost:   800_000 },
        },
    ...
    /* ══════════════════════════════════════════════════════
       ...
       포트폴리오의 animateCounter 패턴을 requestAnimationFrame으로 확장
    ══════════════════════════════════════════════════════ */
    ...
    /* ══════════════════════════════════════════════════════
       ...
       반환: { techs: string[], platform: string|null, complexity: string }
    ══════════════════════════════════════════════════════ */
    ...
    function updateResult(form) {
        ...
        /* 플랫폼 미선택 시 결과 패널 표시하지 않음 (필수 항목) */
        if (!selections.platform) return;
